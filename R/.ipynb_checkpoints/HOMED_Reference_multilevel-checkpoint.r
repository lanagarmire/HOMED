...
#' @import foreach
#' @import parallel
#' @importFrom stats lm model.matrix relevel coef vcov pf
NULL
...
HOMED_Reference <- function(FACS_beta, FACS_pd, training_beta, training_prop, celltypes, p_val = 0.05, numProbes = 500,
    ...
    for (lvl in seq_len(max_level)) {
        ...
        if (lvl == 1) {
            ...
            leaf_types <- colnames(probes_limma_lvl$coefEsts)
            valid_cols <- intersect(leaf_types, colnames(training_prop))
            summarized_props_lvl <- as.data.frame(training_prop[, valid_cols, drop = FALSE])
        }
